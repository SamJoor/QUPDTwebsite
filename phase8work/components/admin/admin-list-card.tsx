      ...
      <div className="mt-6 space-y-4">
        {items.length ? (
          items.map((item, index) => (
            <div
              key={`${item.primary}-${item.secondary}-${item.status}-${index}`}
              className="rounded-2xl border border-fraternity-charcoal/10 bg-white/70 p-4"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-fraternity-burgundy/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-fraternity-burgundy">
                  {item.status}
                </span>
              </div>
            </div>

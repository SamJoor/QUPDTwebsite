  ...
  return (
    <div className="space-y-6">
      <div className="surface p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.25em] text-fraternity-burgundy/80">Private directory</p>
        <h2 className="mt-3 text-4xl">Expanded alumni contact access</h2>
        <p className="mt-4 max-w-3xl text-fraternity-slate">
          This page demonstrates the next privacy layer beyond the public directory by exposing fields that should only appear in authenticated alumni experiences.
        </p>
      </div>
          ...
          <article key={`${profile.name}-${profile.gradYear}`} className="surface p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-fraternity-slate">Class of {profile.gradYear} · {profile.title}, {profile.company}</p>
              </div>
              {profile.mentor ? <span className="rounded-full bg-fraternity-burgundy px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-white">Mentor</span> : null}
            </div>
